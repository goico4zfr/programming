...
public:
    ...
    void expand() {
        ...
        for (int i = 1; i < table_size; i++) {
            if (arr[i].isOccupied()) {
                int key1 = hashFunction(arr[i].getData());
                int index1 = key1;
                while (new_arr[index1].isOccupied()) {
                    index1 = (index1 + hashDelta(arr[i].getData())) % new_size;
                }
                new_arr[index1].setData(arr[i].getData());
            }
        }
        delete[] arr;
        arr = new_arr;
...
int main(){
    ...
    for(int i = 0; i < elements; i++) {
        ...
        probe = myTable.probe(data);
        cout << i+1 << ": " << random_number << "-->" << probe << endl;
